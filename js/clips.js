...
function displayClips(clips) {
  ...
  clips.forEach(function (clip) {
    ...
    let $titleLink = $('<a>')
      .attr('href', 'https://www.reddit.com' + clip.data.permalink)
      .attr('target', '_blank')
    ...
    let $thumbnailLink = $('<a>')
      .addClass('clip-thumbnail')
      .attr('href', '#')
      .append($('<img>').attr('src', thumbnailUrl).attr('alt', clip.data.title))
      .data('clip', clip);
  ...
  $('.clip-thumbnail').click(function (event) {
    ...
    let $iframe = $('<iframe>')
      .attr('src', 'https://clips.twitch.tv/embed?clip=' + slug + '&parent=forsen.site')
      .attr('height', iframeHeight + 'px')
      .attr('width', iframeWidth + 'px')
